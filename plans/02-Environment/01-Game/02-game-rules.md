...
> **Canonical:** `board.would_change(dir)` defined in `02-Environment/02-Rules/03-valid-moves.md` — see there. Do not duplicate `get_valid_moves` logic.

A move is valid iff `board.would_change(direction)` (canonical). The clone+execute snippet below is deprecated reference only:

```rust
// Reference only — canonical is board.would_change(dir)
